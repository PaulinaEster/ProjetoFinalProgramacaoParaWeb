...
if (isset($_POST["botao"]) && $_POST["botao"] == "Cadastrar") { //se clicou no botão Cadastrar
	try {
		...
		$stmt->execute();
		$stmt = $PDO->prepare("insert into categoria (descricao, observacao) values (:descricao, :observacao);");
		$stmt->bindParam(':descricao', $_POST['descricao']);
		$stmt->bindParam(':observacao', $_POST['observacao']);
		$result = $stmt->execute();
		if (!$result) echo "<script>alert('Ocorreu um erro: Categoria não foi cadastrado!')</script>";
		else echo "<script>alert('Categoria cadastrado!')</script>";
...
if (isset($_POST["botao"]) && $_POST["botao"] == "Excluir") { //Se clicou no botão excluir
	...
	if ($linha = $stmt->fetch(PDO::FETCH_ASSOC)) {
		...
		echo "<script> if(confirm('Você deseja excluir definitivamente o " . $linha['descricao'] . "?')) {location.href = 'cadcat.php?acao=ConfirmaExcluir&id=" . $_POST['alterar'] . "';
		 }else{
		   alert('Registro não excluido!');
		   location.href = 'cadcat.php';
		 } </script>";
	}
}
